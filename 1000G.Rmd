...
Now let's look at the distribution of Ti/Tv across the individuals in the population. We use pipeR's `%>>%` operator to express a multi-step data processing pipeline.
...
Gdensifier <- (unpack(SNP_COMMON)
          ...
          %>>% redimension(sprintf("<dense_variant_id:int64> %s", scidb:::build_dim_schema(VARIANT))))
...
Gnormalizer <- (bind(SNP_COMMON, "u", "pow((1-af)*af,0.5)")
           %>>% replaceNA
           %>>% merge(Gdensifier)
           %>>% redimension(sprintf("<u:double> [dense_variant_id=0:%i,1000,0]", count(SNP_COMMON)-1)))
