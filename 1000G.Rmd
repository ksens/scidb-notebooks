...
```{r}
#SNP_COMMON <- (merge(SNP,scidb("KG_VARIANT"))
#          %>>% subset(sprintf("af>= %f and af <=%f ", 0.1, 0.9))
#          %>>% redimension("<reference:string, af:double NULL, chromosome_id: int64, start: int64, end: int64, alternate_id:int64> [variant_id=0:*,10000,0]")
...
G <- (bind(GENOTYPE, "alt_count", "double(iif(allele_1, 1, 0) + iif(allele_2, 1, 0))")
 %>>% merge(Gdensifier)
...
Gnormalizer <- (bind(SNP_COMMON, "u", "pow((1-af)*af,0.5)")
           %>>% replaceNA
           %>>% merge(Gdensifier)
           %>>% redimension(sprintf("<u:double> [dense_variant_id=0:%i,100,0]", count(SNP_COMMON)-1))
           )
