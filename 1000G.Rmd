...
Now let's look at the distribution of Ti/Tv across the individuals in the population.

```{r}
TITV <- bind(GENOTYPE, "alt_copies", "allele1+allele2")
TITV <- bind(merge(TITV, SNP$is_transition), c("ti", "tv"), 
             c("iif(is_transition, alt_copies, 0)",
               "iif(not is_transition, alt_copies, 0)"))
TITV <- aggregate(TITV, FUN="sum(ti) as ti, sum(tv) as tv", by = "sample_id")
TITV <- scidbeval(TITV)[]
...
```{r}
TITV <- merge(GENOTYPE, SAMPLE$sample_name %==% "HG03209")
TITV <- bind(TITV, "alt_copies", "allele1+allele2")
TITV <- bind(merge(TITV, SNP$is_transition), c("ti", "tv"), 
             c("iif(is_transition, alt_copies, 0)",
               "iif(not is_transition, alt_copies, 0)"))
TITV <- aggregate(TITV, FUN="sum(ti) as ti, sum(tv) as tv", by = "sample_id")
...
Ldensifier <- redimension(Ldensifier, sprintf("<dense_variant_id:int64> %s", 
                                              scidb:::build_dim_schema(VARIANT))) 

L <- bind(GENOTYPE, "alt_count", "double(allele1+allele2)")
L <- redimension(merge(L, Ldensifier),
                 sprintf("<alt_count:double NULL> [sample_id = 0:%i,100,0, dense_variant_id=0:%i,100,0]", 
                         count(SAMPLE)-1, count(LD_SNP)-1))
L <- scidbeval(L, temp=TRUE)
```
